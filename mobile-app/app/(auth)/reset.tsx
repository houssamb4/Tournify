...
const ResetPasswordScreen = () => {
  ...
  // Initialize email from route params or authenticated user
  const [email, setEmail] = useState(routeEmail || user?.email || '');
  const [emailEditable, setEmailEditable] = useState(!routeEmail && !user?.email);
  ...
  useEffect(() => {
    // If email is provided via route or auth, move to code verification step
    if (routeEmail || user?.email) {
      handleRequestCode();
    }
  }, []);
  ...
  const handleRequestCode = async () => {
    console.log(`[Reset] Handling reset for email: ${email}`);
